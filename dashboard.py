...
from utils import mysql_query, mysql_prepared_query, mongodb_find, mongodb_aggregate, neo4j_query, plot_collaboration_graph
...
with col2:
    ...
    WHERE f.university_id = u.id AND f.id = fp.faculty_Id AND u.name = %s
    GROUP BY f.id
    ORDER BY num_publications DESC
    LIMIT 10;
    """
    params = (university,)
    prof_data = mysql_prepared_query(query, params)
...
with col3:
    ...
    WHERE f.university_id = u.id AND f.id = fp.faculty_Id AND p.ID = fp.publication_ID AND u.name = %s
    GROUP BY f.id
...
with col4:
    ...
    FROM faculty f, university u
    WHERE f.university_id = u.id AND f.name = %s;
    """
    params = (selected_professor,)
    prof_info = mysql_prepared_query(query, params).iloc[0]
...
with col6:
    ...
    year = st.slider("Select Year", min_value=1948, max_value=2021, value=2015)
    ...
    keywords_data = mongodb_aggregate("publications", pipeline)
